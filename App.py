...
class FinanceApp:
    ...
    def show_debts(self):
        ...
        if st.session_state.debts:
                ...
                if selected_debt:
                    ...
                    with col6:
                        ...
                        if debt_info["状态"] == "还款中":
                            ...
                            if st.button("💳 快速还款", use_container_width=True):
                                new_remaining = debt_info["剩余"] - quick_payment
                                st.session_state.debts[selected_debt]["剩余"] = new_remaining

                                if new_remaining == 0:
                                    st.session_state.debts[selected_debt]["状态"] = "已还清"
                                    st.success(f"🎉 恭喜！{selected_debt} 已全部还清！")
                                else:
                                    st.success(f"✅ 成功还款 {quick_payment:,.2f} 元")

                                self.save_data()
                                st.rerun()

                    with col7:
                        if st.button("🗑️ 删除债务", use_container_width=True, type="secondary", key="delete_debt"):
                            if st.session_state.debts[selected_debt]["剩余"] > 0:
                                st.warning(
                                    f"⚠️ 该债务还有 {st.session_state.debts[selected_debt]['剩余']:,.2f} 元未还清，删除后相关记录将丢失")

                            # 确认删除
                            col_confirm1, col_confirm2 = st.columns([1, 3])
                            with col_confirm1:
                                confirm_delete = st.checkbox(f"确认删除", key=f"confirm_delete_debt_{selected_debt}")
                            with col_confirm2:
                                if confirm_delete:
                                    if st.button("🔥 确认删除债务", use_container_width=True, type="primary"):
            ...
            # 快速初始化示例债务
            if st.button("🚀 快速添加示例债务", use_container_width=True):
                example_debts = {
                    "信用卡欠款": {"总额": 5000, "剩余": 5000, "状态": "还款中", "币种": "人民币",
                                   "创建时间": datetime.now().strftime("%Y-%m-%d %H:%M:%S")},
                    "个人借款": {"总额": 10000, "剩余": 8000, "状态": "还款中", "币种": "人民币",
                                 "创建时间": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
                }
                st.session_state.debts = example_debts
                st.success("✅ 已添加示例债务")
                self.save_data()
                st.rerun()
    ...
    def show_budgets(self):
        ...
        with st.form("add_budget_form"):
            ...
            with col3:
                new_cycle = st.selectbox("预算周期", ["月度", "年度", "周度"])
            with col4:
            ...
            if add_submitted:
                if new_category and new_category.strip():
                    if new_category not in st.session_state.budgets:
                        st.session_state.budgets[new_category] = {
                            "预算金额": new_amount,
                            "已用金额": 0,
                            "周期": new_cycle,
                            "币种": new_currency
                        }
                        st.success(f"✅ 成功添加预算类别: {new_category}")
                        self.save_data()
                        st.rerun()
                    else:
                        st.error("❌ 该预算类别已存在")
        ...
        if st.session_state.budgets:
            ...
            if not budget_df.empty:
                ...
                st.dataframe(
                    display_df[["类别", "币种", "预算金额", "已用金额", "剩余金额", "使用进度", "状态", "周期"]],
                    use_container_width=True
                )
                ...
                with col1:
                    edit_categories = list(st.session_state.budgets.keys())
                    selected_category = st.selectbox("选择要编辑的预算类别", edit_categories)

                if selected_category:
                    budget_info = st.session_state.budgets[selected_category]
                    ...
                    with col3:
                        new_budget_cycle = st.selectbox(
                            "新预算周期",
                            ["月度", "年度", "周度"],
                            index=["月度", "年度", "周度"].index(budget_info.get("周期", "月度")),
                            key="edit_cycle"
                        )
                    ...
                    with col5:
                        if st.button("✅ 更新预算", use_container_width=True):
                            st.session_state.budgets[selected_category]["预算金额"] = new_budget_amount
                            st.session_state.budgets[selected_category]["周期"] = new_budget_cycle
                            st.session_state.budgets[selected_category]["币种"] = new_budget_currency
                            st.success(f"✅ 成功更新 {selected_category} 的预算")
                            self.save_data()
                            st.rerun()

                    with col6:
                        if st.button("🗑️ 删除预算", use_container_width=True, type="secondary"):
                            if st.session_state.budgets[selected_category]["已用金额"] > 0:
                                st.warning(
                                    f"⚠️ 该预算类别已有 {st.session_state.budgets[selected_category]['已用金额']} 元的使用记录，删除后相关交易记录将不再计入预算统计")

                            if st.checkbox(f"我确认要删除 '{selected_category}' 预算",
                                           key=f"confirm_delete_{selected_category}"):
                                del st.session_state.budgets[selected_category]
                ...
                st.subheader("🥧 预算分布")
                chart_data = []
                for category, info in st.session_state.budgets.items():
                    chart_data.append({
                        "类别": category,
                        "预算金额": info["预算金额"],
                        "币种": info.get("币种", "人民币")
            ...
            # 快速初始化默认预算
            if st.button("🚀 快速初始化默认预算", use_container_width=True):
                default_budgets = {
                    "餐饮": {"预算金额": 1000, "已用金额": 0, "周期": "月度", "币种": "人民币"},
                    "购物": {"预算金额": 2000, "已用金额": 0, "周期": "月度", "币种": "人民币"},
                    "交通": {"预算金额": 500, "已用金额": 0, "周期": "月度", "币种": "人民币"},
                ...
                st.session_state.budgets = default_budgets
                st.success("✅ 已初始化默认预算")
                self.save_data()
                st.rerun()
